...
const CreateRoom = async (req, res) => {
  try {
    ...
    // console.log({date})
    database.query(
      `INSERT INTO meetings VALUES('${room}','${formattedDate}','${organiser}','${password}') ;`,
      async (err, result) => {
        // console.log(result)
        console.log(err);
        if (err) res.status(500).json({ message: err.message });
          ...
          res
            .status(200)
            .json({ created: true, message: "Succesfully Booked Session" });
...
const StartRoom = async ({ organiserId, room, password, socket }) => {
  try {
    ...
    database.query(
      `SELECT COUNT(room_id) FROM meetings WHERE room_id=? AND organiser_id=? AND password=?;`,
      ...
      (err, result) => {
        ...
        if (result?.length > 0) {
          if (+result[0]["COUNT(room_id)"] === 1) {
            ...
            socket.broadcast
              .to(room)
              .emit("room:start", { message: "Meet Started" });
            return;
          }
        ...
        socket.emit("room:error", {
          message: "Meeting Not Started OR Wrong Details",
        });
      }
    );
  } catch (error) {
...
const JoinRoom = ({ room, password, user, socket }) => {
  try {
    ...
    database.query(
      `SELECT COUNT(room_id) FROM meetings WHERE room_id=? AND password=?;`,
      [room, password],
      (err, result) => {
        if (err) {
